...
CREATE TABLE shane_ud1_roads.osm_roads AS (
	SELECT *
...
-- arnold lines in ud1 --
CREATE TABLE shane_ud1_roads.arnold_lines AS (
	SELECT *
...
CREATE TABLE shane_ud1_roads.osm_roads_add_lanes AS (
...
SELECT * FROM shane_ud1_roads.osm_roads
WHERE osm_id NOT IN (
...
-- WHILE LOOP TO RUN MORE THAN ONCE, CURRENTLY COUNT = 0
	-- in this case, we don't need to run because there aren't any osm roads without lane count
	-- we will need this in the future when conflating the whole dataset though
INSERT INTO shane_ud1_roads.osm_roads_add_lanes(osm_id, osm_road_name, lanes, geom)
WITH ranked_road AS (
	SELECT 
		r2.osm_id AS osm_id, 
	...
	FROM shane_ud1_roads.osm_roads_add_lanes r1
	JOIN (
...
SELECT 
	...
	osm_geom
FROM ranked_road
WHERE RANK = 1; -- count: 0


-- checkpoint --
	-- what's in? --
SELECT * FROM shane_ud1_roads.osm_roads_add_lanes
	-- what's not in? --
SELECT * FROM shane_ud1_roads.osm_roads
WHERE osm_id NOT IN (
	SELECT osm_id FROM shane_ud1_roads.osm_roads_add_lanes
...
CREATE TABLE shane_ud1_roads.conflation_road_case AS 
WITH ranked_roads AS (
	...
	FROM shane_ud1_roads.osm_roads_add_lanes AS osm
	RIGHT JOIN shane_ud1_roads.arnold_lines AS arnold
...
-- checkpoint --
	-- what's in? --
SELECT * FROM shane_ud1_roads.conflation_road_case
	-- what's not in? --
SELECT * FROM shane_ud1_roads.osm_roads
WHERE osm_id NOT IN (
	SELECT osm_id FROM shane_ud1_roads.conflation_road_case crc 
);

SELECT * FROM shane_ud1_roads.arnold_lines 
...
--INSERT INTO shane_ud1_roads.conflation_road_case (
--	osm_id, 
--	osm_road_name,
--	lanes,
--	osm_geom, 
...
--FROM shane_ud1_roads.osm_roads_add_lanes lanes
--JOIN shane_ud1_roads.conflation_road_case road_case
...
--	SELECT osm_id
--	FROM shane_ud1_roads.conflation_road_case
--); -- count: 15

	
-- check point -- 
SELECT * FROM shane_ud1_roads.conflation_road_case -- count: 30
