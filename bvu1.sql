...
INSERT INTO shane.bvu1_conflation_crossing_case (osm_label, osm_id, osm_geom, arnold_road_id, arnold_road_geom, arnold_road_shape)
	...
	JOIN shane.bvu1_arnold_segments AS road ON ST_Intersects(crossing.geom, road.geom);
...
WHERE shape NOT IN (
    ...
    SELECT shape FROM shane.bvu1_conflation_edge_case
) AND shape NOT IN (
	SELECT shape FROM shane.bvu1_conflation_connecting_link_case
);
