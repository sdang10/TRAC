...
-- OSM sidewalks --
CREATE TABLE shane_ch1_sw.osm_sw AS (
	SELECT *
...
-- OSM points --
CREATE TABLE shane_ch1_sw.osm_point AS (
	SELECT *
...
-- OSM crossings -- 
CREATE TABLE shane_ch1_sw.osm_crossing AS (
	SELECT *
	FROM shane_data_setup.osm_lines
	WHERE   highway = 'footway'
...
-- ARNOLD roads --
CREATE TABLE shane_ch1_sw.arnold_lines AS (
	SELECT *
...
-- OSM footway NULL --
CREATE TABLE shane_ch1_sw.osm_footway_null AS (
	SELECT *
	FROM shane_data_setup.osm_lines
	WHERE 	highway = 'footway' AND 
			(tags -> 'footway' IS NULL OR 
			tags -> 'footway' NOT IN ('sidewalk', 'crossing'))  AND
			geom && st_setsrid( st_makebox2d( st_makepoint(-13617414,6042417), st_makepoint(-13614697,6045266)), 3857)
); -- count: 175
...
CREATE TABLE shane_ch1_sw.conflation_entrance_case (
	osm_label TEXT,
	osm_sw_id INT8,
...
INSERT INTO shane_ch1_sw.conflation_entrance_case (
	osm_label, 
	osm_sw_id, 
	osm_sw_geom, 
	osm_point_id, 
...
JOIN (
	SELECT *
	FROM shane_ch1_sw.osm_point AS point
	WHERE tags -> 'entrance' IS NOT NULL
) AS point 
	ON ST_intersects(sw.geom, point.geom); -- count: 1


SELECT * FROM shane_ch1_sw.conflation_entrance_case
...
CREATE TABLE shane_ch1_sw.conflation_crossing_case (
	osm_label TEXT,
	osm_id INT8,
...
INSERT INTO shane_ch1_sw.conflation_crossing_case (
	osm_label, 
	osm_id, 
	osm_geom, 
	arnold_route_id, 
...
FROM shane_ch1_sw.osm_crossing AS crossing
JOIN shane_ch1_sw.arnold_lines AS road 
	ON ST_Intersects(crossing.geom, road.geom); -- count 1051
...
WHERE crossing.osm_id NOT IN (
	SELECT osm_id
	FROM shane_ch1_sw.conflation_crossing_case 
); -- count: 0
...
--- check point ---
	-- what's in
SELECT * FROM shane_ch1_sw.conflation_crossing_case
	-- what's not in
SELECT *
FROM shane_ch1_sw.osm_crossing
WHERE osm_id NOT IN (
    SELECT osm_id FROM shane_ch1_sw.conflation_crossing_case
);
...
CREATE TABLE shane_ch1_sw.osm_connecting_links AS (
	SELECT DISTINCT sw.*
	FROM shane_ch1_sw.osm_sw AS sw
	JOIN shane_ch1_sw.osm_crossing AS crossing
...
CREATE TABLE shane_ch1_sw.osm_footway_null_connecting_links AS (
	SELECT DISTINCT fn.*
	FROM shane_ch1_sw.osm_footway_null AS fn
	JOIN shane_ch1_sw.osm_crossing AS crossing
...
CREATE TABLE shane_ch1_sw.conflation_connecting_link_case (
	osm_label TEXT,
	osm_cl_id INT8,
...
INSERT INTO shane_ch1_sw.conflation_connecting_link_case (
    ...
    crossing.arnold_route_id AS arnold_route_id,
    crossing.arnold_road_geom AS arnold_road_geom
FROM shane_ch1_sw.osm_footway_null_connecting_links AS cl
JOIN shane_ch1_sw.conflation_crossing_case AS crossing
    ON ST_Intersects(cl.geom, crossing.osm_geom); -- count: 1

    
-- checkpoint --
    -- what's in?
SELECT * FROM shane_ch1_sw.conflation_connecting_link_case
	-- what's not in?
SELECT * FROM shane_ch1_sw.osm_connecting_links
WHERE osm_id NOT IN (
...
CREATE TABLE shane_ch1_sw.conflation_general_case AS
...
WHERE osm_id NOT IN (
	...
	SELECT osm_id FROM shane_ch1_sw.conflation_general_case
);
...
CREATE TABLE shane_ch1_sw.conflation_corner_case (
	osm_label TEXT,
    osm_id INT8,
    osm_geom GEOMETRY(LineString, 3857),
...
WHERE osm_id NOT IN (
	...
	SELECT osm_id FROM shane_ch1_sw.conflation_corner_case
);
...
UNION
SELECT osm_cl_geom
FROM shane_ch1_sw.conflation_connecting_link_case
UNION
SELECT osm_geom
FROM shane_ch1_sw.conflation_general_case
UNION
...
SELECT *
FROM shane_ch1_sw.arnold_lines
WHERE route_id NOT IN (
    SELECT arnold_route_id FROM shane_ch1_sw.conflation_general_case
) AND route_id NOT IN (
    SELECT arnold_road1_route_id FROM shane_ch1_sw.conflation_corner_case
) AND shape NOT IN (
	SELECT arnold_road2_route_id FROM shane_ch1_sw.conflation_corner_case
) AND shape NOT IN (
	SELECT arnold_route_id FROM shane_ch1_sw.conflation_connecting_link_case
); -- count: 21
...
CREATE TABLE shane_ch1_sw.weird_cases AS
...
CREATE TABLE shane_ch1_sw.weird_case_segments AS
	WITH segments AS (
		SELECT osm_id,
		ROW_NUMBER() OVER (
		   PARTITION BY osm_id 
		...
		FROM (SELECT osm_id, ST_DumpPoints(geom) AS pt FROM shane_ch1_sw.weird_cases) AS dumps )
	SELECT * FROM segments WHERE geom IS NOT NULL; --count: 162
	

-- segments that are connecting links
CREATE TABLE shane_ch1_sw.weird_case_connecting_link_case AS
	WITH connlink_rn AS (
		SELECT DISTINCT ON 
	    ...
	    FROM shane_ch1_sw.conflation_crossing_case AS crossing
	    JOIN shane_ch1_sw.weird_case_segments AS link
...
CREATE TABLE shane_ch1_sw.weird_case_general_case AS
WITH ranked_roads AS (
	SELECT
	...
	FROM shane_ch1_sw.weird_case_segments AS sidewalk
	JOIN shane_ch1_sw.arnold_lines AS big_road ON ST_Intersects(ST_Buffer(sidewalk.geom, 5), ST_Buffer(big_road.geom, 18))
	WHERE (
    ...
    AND (sidewalk.osm_id, sidewalk.osm_segment_number) NOT IN (
    	SELECT osm_cl_id, osm_segment_number
      	FROM shane_ch1_sw.weird_case_connecting_link_case
    )
)
SELECT
	'sidewalk' AS osm_label,
...
INSERT INTO shane_ch1_sw.weird_case_general_case (osm_label, osm_id, osm_segment_number, arnold_route_id, osm_geom, arnold_geom)
WITH min_max_segments AS (
	SELECT 
		...
		sw.arnold_route_id
	FROM shane_ch1_sw.weird_case_general_case AS sw
	JOIN shane_ch1_sw.weird_case_connecting_link_case AS link
...
SELECT 
	...
	ST_LineSubstring( road.geom, LEAST(ST_LineLocatePoint(road.geom, ST_ClosestPoint(st_startpoint(seg_sw.geom), road.geom)) ,
		...
		road.geom))) ) AS seg_geom
FROM shane_ch1_sw.weird_case_segments AS seg_sw
...
AND (seg_sw.osm_id, seg_sw.osm_segment_number) NOT IN (
	SELECT 
		osm_id, 
		osm_segment_number
	FROM shane_ch1_sw.weird_case_general_case
) AND (seg_sw.osm_id, seg_sw.osm_segment_number) NOT IN (
	SELECT 
		osm_cl_id, 
		osm_segment_number
...
FROM shane_ch1_sw.weird_case_segments seg_sw
JOIN min_max_segments mms 
	ON seg_sw.osm_id = mms.osm_id
JOIN shane_ch1_sw.arnold_lines  road
	ON mms.arnold_route_id = road.route_id
WHERE seg_sw.osm_segment_number BETWEEN mms.min_segment AND mms.max_segment
AND (seg_sw.osm_id, seg_sw.osm_segment_number) NOT IN (
...
WITH ranked_road AS (
	...
	FROM shane_ch1_sw.weird_case_segments osm_sw
	JOIN shane_ch1_sw.weird_case_general_case sidewalk
		ON 	ST_Intersects(st_startpoint(sidewalk.osm_geom), st_startpoint(osm_sw.geom))
		OR ST_Intersects(st_startpoint(sidewalk.osm_geom), st_endpoint(osm_sw.geom))
		OR ST_Intersects(st_endpoint(sidewalk.osm_geom), st_startpoint(osm_sw.geom))
		OR ST_Intersects(st_endpoint(sidewalk.osm_geom), st_endpoint(osm_sw.geom))
	JOIN shane_ch1_sw.arnold_lines big_road
		ON ST_Intersects(ST_Buffer(osm_sw.geom, 5), ST_Buffer(osm_sw.geom, 18))
	WHERE (osm_sw.osm_id, osm_sw.osm_segment_number) NOT IN (
		SELECT sw.osm_id, sw.osm_segment_number FROM shane_ch1_sw.weird_case_general_case AS sw
		UNION ALL 
		SELECT link.osm_cl_id, link.osm_segment_number FROM shane_ch1_sw.weird_case_connecting_link_case AS link
	) AND ( -- specify that the segment should be PARALLEL TO our conflated sidewalk
		ABS(DEGREES(ST_Angle(sidewalk.osm_geom, osm_sw.geom))) BETWEEN 0 AND 20 -- 0 
		OR ABS(DEGREES(ST_Angle(sidewalk.osm_geom, osm_sw.geom))) BETWEEN 160 AND 200 -- 180
		OR ABS(DEGREES(ST_Angle(sidewalk.osm_geom, osm_sw.geom))) BETWEEN 340 AND 360  ) -- 360 
...
-- checkpoint --
SELECT * FROM shane_ch1_sw.weird_case_general_case




-- Step 3: Deal with corner
CREATE TABLE shane_ch1_sw.weird_case_corner_case AS
	...
	FROM shane_ch1_sw.weird_case_segments corner
	JOIN (	SELECT * 
			FROM shane_ch1_sw.weird_case_general_case
			UNION ALL
	...
	WHERE   (corner.osm_id, corner.osm_segment_number) NOT IN (
					SELECT sw.osm_id, sw.osm_segment_number FROM shane_ch1_sw.weird_case_general_case sw
					UNION ALL 
					SELECT link.osm_cl_id, link.osm_segment_number FROM shane_ch1_sw.weird_case_connecting_link_case link
				)
...
WHERE osm_id NOT IN (
	...
	SELECT osm_id FROM shane_ch1_sw.weird_case_general_case
) AND osm_id NOT IN (
    SELECT osm_id FROM shane_ch1_sw.weird_case_corner_case
) AND osm_id NOT IN (
	SELECT osm_cl_id FROM shane_ch1_sw.weird_case_connecting_link_case
);
